...
export function AssignToSlotSheet({ open, onOpenChange, assignment }: AssignToSlotSheetProps) {
    ...
    const handleSelect = (blockId: string) => {
        assignToSlot.mutate(
            { id: assignment.id, schedule_block_id: blockId },
                ...
                {isLoading ? (
                    ...
                    <Empty className="py-6 border border-dashed rounded-xl">
                        <EmptyHeader>
                            ...
                            <EmptyDescription className="text-[11px] max-w-xs">
                                Every free block today is either too short or already full. Leave this task unslotted,
                                or free up a block.
                            </EmptyDescription>
                        </EmptyHeader>
                    </Empty>
                    ...
                    <div className="flex flex-col gap-1 -mx-1">
                        {availableSlots.map((block) => {
                            const remaining = getBlockRemainingMinutes(block, dayAssignments, assignment.id);
                            return (
                                <button
                                    key={block.id}
                                    type="button"
